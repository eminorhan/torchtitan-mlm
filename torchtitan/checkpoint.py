...
class ModelWrapper(Stateful):
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        func = functools.partial(set_model_state_dict, model_state_dict=state_dict, options=StateDictOptions(strict=False))
...
class OptimizerWrapper(Stateful):
    ...
    def state_dict(self) -> None:
        func = functools.partial(get_optimizer_state_dict, options=StateDictOptions(flatten_optimizer_state_dict=True))
        return {k: v for sd in map(func, self.model, self.optim) for k, v in sd.items()}

    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        func = functools.partial(set_optimizer_state_dict, optim_state_dict=state_dict, options=StateDictOptions(flatten_optimizer_state_dict=True))
...
def checkpoint_mp(recv, send):
    ...
    try:
        while True:
            ...
            logger.info(f"Finish saving the checkpoint in the background process in {time.monotonic() - begin:.2f} seconds.")
    finally:
...
class CheckpointManager:
    ...
    def _save_last_step(self, curr_step: int) -> None:
        ...
        if self.model_weights_only:
            ...
            logger.info(f"Saving a model weights only checkpoint in {self.export_dtype} at last step, step {curr_step}.")
        else:
    ...
    def save(self, curr_step: int, force: bool = False) -> None:
        ...
        logger.info(f"Finished saving the checkpoint (or staging if async is enabled) in {time.monotonic() - begin:.2f} seconds.")
