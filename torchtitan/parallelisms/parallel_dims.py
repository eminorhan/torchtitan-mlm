...
class ParallelDims:
    ...
    def _validate(self):
        ...
        assert tp >= 1, tp
        assert pp >= 1, pp
        assert dp_replicate * dp_shard * tp * pp == self.world_size, f"Invalid parallel dims: dp_replicate({dp_replicate}) * dp_shard({dp_shard}) * tp({tp}) * pp({pp}) != WORLD_SIZE({self.world_size})"

    def build_mesh(self, device_type):
        dims = []
        names = []
        for d, name in zip([self.pp, self.dp_replicate, self.dp_shard, self.tp], ["pp", "dp_replicate", "dp_shard", "tp"], strict=True):
            if d > 1:
                dims.append(d)
